...
def load_pe(lens: str) -> dict[int, list]:
    """P/E points per series id for one lens. A missing CSV drops that series."""
    out = {}
    for sid in SERIES:
        pts = _round_series(_load_csv_points(pe_csv(lens, sid)), 4)
        if pts:
            out[sid] = pts
    return out
...
def _families():
    """Forward and trailing family payloads (None when a lens has no data)."""
    out = {}
    for lens in ("forward", "trailing"):
        pts = load_pe(lens)
        out[lens] = build_family_payload(pts) if pts else None
    return out


def table_brief(fams=None) -> tuple[str, set[str], str]:
...
def build() -> Path:
    fams = _families()
    forward, trailing = fams["forward"], fams["trailing"]
    spx_forward_pe = next((x["points"] for x in forward["series"] if x["id"] == 20052), [])
    ...
    fg_points = _round_series(_load_csv_points(FEAR_GREED_CSV), 2)
    spx_points = _round_series(_load_csv_points(SPX_PRICE_CSV), 2)
    us10y_points = _round_series(_load_csv_points(US10Y_CSV), 3)
